...
public class FlexChannelPortlet extends ActionsoftWeb
{
  public static final int TITLE_LENGTH = 20;

  public FlexChannelPortlet(UserContext uc)
  {
    super(uc);
  }
    ...
    throws UnsupportedEncodingException
  {
    ...
    return RepleaseKey.replace(HtmlModelFactory.getHtmlModel("portlet_FlexCMSChannel.htm"), hashTags);
  }
  ...
  private String getTabHtml(String sid, String top, String threetop, int tabCount, Vector channels, Vector tabs, Vector channelNames, String isMorePage, String start, String isDepartment, String channelConf, int m)
  {
    ...
    if (tabCount == 1) {
      ...
      html.append("<thead>");
      html.append("<tr>");
      html.append("<th field='title'width='75%' formatter='formatGridTitle'><I18N#标题></th>");
     ...
     //将更过改为英文的MORE
      html.append("<a   href='javascript:void(0);' onclick=\"openLocation('./login.wf?sid=").append(super.getContext().getSessionId()).append("&cmd=Portal_Execute_Portlet_FlexCMSChannelMore&top=").append(threetop).append("&channelTab=").append(tabs.get(0).toString()).append("&channelConf=").append(channelNames.get(0).toString()).append("&isDepartment=").append(isDepartment).append("&isMorePage=").append(isMorePage).append("','mainFrame','").append(tabs.get(0).toString()).append("');return false;\" >MORE</a>\n");
      /**
       * 跳转到二级页面
       */
     // html.append("<a   href='javascript:void(0);' onclick=\"openLocation('./login.wf?sid=").append(super.getContext().getSessionId()).append("&cmd=TwoPages&top=").append(threetop).append("&channelTab=").append(tabs.get(0).toString()).append("&channelConf=").append(channelNames.get(0).toString()).append("&isDepartment=").append(isDepartment).append("&isMorePage=").append(isMorePage).append("','mainFrame','").append(tabs.get(0).toString()).append("');return false;\" >[<I18N#更多>]</a>\n");
      ...
      if ((isAc) && (SecurityProxy.checkWorkFlowInUser(super.getContext().getUID(), flowModel._id))) {
    	  //发布名字更改为加号
        html.append("<div  id='public' style='float:right;  padding-top:-2px'><a href='#' onclick=\"openPublic('./login.wf?sid=" + super.getContext().getSessionId() + "&cmd=Portal_Execute_Portlet_QuickCMS_Create&channelId=" + channels.get(0) + "&title=','');return false;\">+</a></div>\n");
      }
