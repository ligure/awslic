...
/**
 * 
 * @description 修改CMS新建后，人在乐视页面的修改。
 * @version 1.0
 * @author wangaz
 * @update 2014-1-6 上午10:39:10
 */
public class LeopleLS extends ActionsoftWeb
{
  public static final int TITLE_LENGTH = 20;

  public LeopleLS(UserContext uc)
  {
    super(uc);
  }
    ...
    throws UnsupportedEncodingException
  {
    ...
    for (int i = 0; i < tmpChannels.size(); i++) {
      ...
      tmpList = new UtilString(tmpChannel).split(",");
      String channelIds = "";
      ...
      if (tabs.size() == 1) {
        html.append(getTabHtml(sid, top, threetop, 1, channels, tabs, channelNames, isMorePage, start, isDepartment, channelConf, 0));
         if("人在乐视".equals(channelConf)){
        	 ...
        	 /**
      	      * 修改栏目图片内容，将图片显示展示在html页面上
      	      */
        	 titleimg.append("<div align='center' class='boxx1'><img style='width:288px;height:141px;' name=subPicture onclick='gridTitleClick("+id+",30)'  src ='./downfile.wf?flag1=")
		        ...
		        .append(filename).append("' border=0></div>");
      	       ...
      	       // titleimg.append("<div class='boxx1'><img border='0' style='width:288px;height:141px;'src='./downfile.wf?flag1=244541&flag2=10460&sid="+super.getContext().getSessionId()+"&filename="+filename+"&rootDir=FormFile' alt='111111111111111111111111111111' title='' rel=''/></a></div>");
      	      // titleimg.append("<img name='subPicture' onload='javascript:if(this.width&gt;110){this.width=110;};' src='./downfile.wf?flag1=119331&amp;flag2=11097&amp;sid="+super.getContext().getSessionId()+"&amp;rootDir=FormFile&amp;filename=aaa.jpg' border='0' width='110'>");
      	        /**
      	       * *********************************************结束*********************************************
      	       */
        }	
    ...
    //新增图片
    hashTags.put("titimg", titleimg.toString());
    hashTags.put("title", channelConf.toString());
    ...
    return RepleaseKey.replace(HtmlModelFactory.getHtmlModel("letv_LeopleLS.htm"), hashTags);
  }
  ...
  private String getTabHtml(String sid, String top, String threetop, int tabCount, Vector channels, Vector tabs, Vector channelNames, String isMorePage, String start, String isDepartment, String channelConf, int m)
  {
    ...
    if (tabCount == 1) {
      ...
      html.append("<div   dojoType='dijit.layout.BorderContainer' style='background:none;width:100%; height:80%; border:0px; padding:0px; margin:0px;'>\n");
      html.append("<div  dojoType='dijit.layout.ContentPane' region='center'  style='background:none; width:100%; height:100%; border:0px; padding:0px; margin:0px;'>\n");
      html.append(" <span style='background:none;' dojoType='dojo.data.ItemFileWriteStore'  jsId='jsonStore'  url='./login.wf?sid=").append(sid).append("&cmd=Portal_Execute_Portlet_FlexCMSChannelGetListData&top=").append(top).append("&channelConf=").append(channels.get(0).toString()).append("&start=").append(start).append("&isDepartment=").append(isDepartment).append("&isMore=").append(isMorePage).append("'> </span>");
      html.append("<table style='background:none;' class='box' dojoType='dojox.grid.DataGrid' jsid='grid' id='grid' store='jsonStore' rowsPerPage='20' rowSelector='0px' noDataMessage='<I18N#无信息>'>");
      html.append("<thead>");
      html.append("<tr>");
      html.append("<th field='title'width='100%' formatter='formatGridTitle'><I18N#标题></th>");
      //html.append("<th field='date' width='100px' ><I18N#日期></th>");
      //html.append("<th field='name' width='56px' ><I18N#发布人></th>");
      //html.append("<th field='departmentName' width='56px' ><I18N#部门></th>");
      html.append("</tr>\n");
      html.append("</thead>\n");
      html.append("</table>\n");
      html.append("</div>\n");
      html.append("<div dojoType='dijit.layout.ContentPane' region='bottom' style='background:none; width:100%; border:0px; padding:0px; margin:0px;'>\n");
      html.append("<div id='moreTaskEntrance' style='background:none; float:right;  padding-top:-2px'>");
     /**
      * 是否是人在乐视
      * 跳转修改后的二级页面
      */
      ...
      if ((isAc) && (SecurityProxy.checkWorkFlowInUser(super.getContext().getUID(), flowModel._id))) {
        html.append("<div  id='public' style='margin:0 0 5px 0;float:right;  padding-top:-2px'><a href='#' onclick=\"openPublic('./login.wf?sid=" + super.getContext().getSessionId() + "&cmd=Portal_Execute_Portlet_QuickCMS_Create&channelId=" + channels.get(0) + "&title=','');return false;\">[<I18N#发布>]</a></div>\n");
      }
      html.append("<div id='page'>\n");
      String td1 = "<td align='center' valign='middle' nowrap='nowrap'>";
      String td2 = "</td>\n";

      html.append("<div dojoType=\"dijit.Toolbar\" id=\"dojo_pagebar_toolbar\" style=\"background:none;padding-top:3px;\">");
      html.append("<table style='background:none;' border='0' cellspacing='0' cellpadding='0'>\n<tr>");
      html.append(td1).append("<div style='background:none;' id=\"dojo_pagebar_first\" dojoType=\"dijit.form.Button\" title='<I18N#首页>'></div>").append(td2);
      html.append(td1).append("<div style='background:none;' id=\"dojo_pagebar_pre\" dojoType=\"dijit.form.Button\" title='<I18N#上一页>'></div>").append(td2);
      html.append(td1).append("<div style='background:none;' dojoType=\"dijit.ToolbarSeparator\"></div>\n").append(td2);
      html.append(td1).append("&nbsp;<I18N#第>").append(td2);
      html.append(td1).append("<div style='background:none;' dojoType=\"dijit.form.TextBox\" id=\"dojo_pagebar_textbox\" style='text-align:center;width:20px;margin:2xp 6px 0px 6px;' title='<I18N#请输入页数>'></div>").append(td2);
      html.append(td1).append("<I18N#页>&nbsp;<I18N#共><span id='dojo_pagebar_pagecount'>0</span><I18N#页>&nbsp;").append(td2);
      html.append(td1).append("<div style='background:none;' dojoType=\"dijit.ToolbarSeparator\"></div>\n").append(td2);
      html.append(td1).append("<div style='background:none;' id=\"dojo_pagebar_next\" dojoType=\"dijit.form.Button\" title='<I18N#下一页>'></div>").append(td2);
      html.append(td1).append("<div style='background:none;' id=\"dojo_pagebar_end\" dojoType=\"dijit.form.Button\" title='<I18N#尾页>'></div>").append(td2);
      html.append("<td style='background:none;' align='center' valign='middle' nowrap='nowrap' width='100%'></td>\n");
      html.append(td1).append("<div style='background:none;' id=\"dojo_pagebar_info\"></div>").append(td2);
      html.append("</tr>\n");
      html.append("</table>\n");
      html.append("</div>\n");
      ...
      html.append("<div  style='background:none;' dojoType='dijit.layout.ContentPane'  title='").append(I18nRes.findValue(super.getContext().getLanguage(), tabs.get(m).toString())).append("'  style='width:100%; height:100%; border:0px; padding:0px; margin:5px;'>");
      html.append("<div style='background:none;' dojoType='dijit.layout.BorderContainer' style='width:100%; height:100%; border:0px; padding:0px; margin:0px;'>\n");
      html.append("<div  style='background:none;' dojoType='dijit.layout.ContentPane' region='center' style='width:100%; border:0px; padding:0px; margin:0px;'>\n");
      ...
      html.append("<table style='background:none;' dojoType='dojox.grid.DataGrid' jsid='grid").append(m).append("' id='grid").append(m).append("' store='jsonStore' rowsPerPage='20' rowSelector='0px' noDataMessage='<I18N#无信息>'>\n");
      html.append("<thead>\n");
      html.append("<tr>\n");
      html.append("<th field='title' width='45%'formatter='formatGrid").append(m).append("Title'><I18N#标题></th>\n");
      html.append("<th field='date' width='140px' ><I18N#日期></th>\n");
      html.append("<th field='name' width='56px' ><I18N#发布人></th>\n");
      html.append("<th field='departmentName' width='56px' ><I18N#部门></th>");
      html.append("</tr>\n");
      html.append("</thead>\n");
      html.append("</table>\n");
      html.append("</div>\n");
      html.append("<div style='background:none;' dojoType='dijit.layout.ContentPane' region='bottom' style='width:100%; border:0px; padding:0px; margin:0px;'>\n");
      html.append("<div style='background:none;' id='moreTask").append(m).append("Entrance' style='float:right;'>\n");
