 ...
 */
public class SystemSocketCommand implements BaseSocketCommand
{
...
//---------------------------------HttpClient Startup--------------------------------------------------------------------------//
     //value为页面中传过来的cookie值
       ...
       if("mysid".equals(uid)){
    	   ...
    	   String usid = sid[0];
    	  boolean flag = SessionAPI.getInstance().checkSession(usid, pwd);
    	  if(flag){
    		  value=usid;
    	  }else{
    	      myOut.write("<script language='javascript'>window.location.href='" +  ApplictionUtil.getProperty("SSO_URL") + "';</script>");
    	      return true;
    	  }
       }else{
    	 //调用客户方API解析cookie里面的值，最终获得用户名
      		...
      		try{
      			...
      			value = objects.substring(begin, end);
      		    } else {
      			myOut.write("<script language='javascript'>window.location.href='" +  ApplictionUtil.getProperty("SSO_URL") + "';</script>");
      		    }			
      		}catch(Exception e){
