...
public abstract class DictionaryObjectBak extends ActionsoftWeb {
	...
	while (bindValue.length() > 0 && bindValue.indexOf("{") > -1) {
	    String tmp1 = bindValue.substring(0, bindValue.indexOf("}") + 1);
	    bindValue = bindValue.substring(bindValue.indexOf("}") + 1);
	    bindValue = bindValue.substring(bindValue.indexOf("_") + 1).trim();
	    String key = new UtilString(tmp1).matchValue("_", "{");
	    String value = new UtilString(tmp1).matchValue("{", "}");
	    if (key.length() > 0)
		this._webFormData.put(key, value);
	}
    }

    public DictionaryObjectBak(UserContext userContext) {
	super(userContext);
    }
